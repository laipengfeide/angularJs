define([ "jquery" ], function($) {
	...
	function initAccount(account) {
		login.user = new User(account.name, account.lastSeen, account.saving.currency, account.note);
		login.savings = new Savings(account.saving.amount, account.saving.deposit, account.saving.capitalization, account.saving.interest);
	...
	function initGreetingPage() {
		...
		initStatisticPage();
		setTimeout(function() {
			initSavingsCircles(login.user.checkedPercent, 0.2, login.savings.freeMoney, login.savings.freeMoney, 700)
		}, 200);
		initSavingsSlider();
		$("#savings-slider").data({
			"checkedPercent" : login.user.checkedPercent
		});

		$("#lefttitle").prepend(escape(login.user.login));
		$("#righttitle").append(login.user.lastSeen);
	...
	function initSettingsPage() {
		...
		hideGreetingUnits();
		setTimeout(function() {
			$("#logo_greeting").hide(); $("#logo_settings").show(); $("#avatarcontainer").fadeOut(100); drawChartLine(91);
		}, 290);
		setTimeout(function() {
			$("#settingspage").fadeIn(100);
		}, 700);
		setTimeout(function() {
			$("#expenseslider").fadeIn(100); $("#bubble").fadeIn(500);
		}, 800);
	}
	function greetingPageAgain() {
		...
		$("#righttitle").append('<span class="bluetext">last seen: </span>' + login.user.lastSeen);
		$("#lefttitle").append(escape(login.user.login) + '<span class="bluetext"> metrics</span>');
	}
	...
	// Filling user notes
	function addNotes() {
		$("textarea#notes").val(login.user.notes);
	}

	// Filling Savings column
	function addSavings() {
		switch (login.savings.deposit) {
		case true:
			$("#deposit").prop('checked', true);
			break;
		}
		switch (login.savings.capitalization) {
		case true:
			$("#capitalization").prop('checked', true);
			break;
		}
		$("#savingsvalue").val(login.savings.freeMoney);
		$("#percentvalue").val(login.savings.percent);
	...
	function itemClick(item) {
		...
		deleteItem = function() {
			...
			setTimeout(function() {
				$("#" + incomesExpenses + "-" + itemId).remove(); checkSlidersLength()
			}, 300);
	...
	function turnOffModal() {
		...
		$(".modalincomessurface, .modalexpensessurface").fadeOut(150);
		setTimeout(function() {
			$(".modalvalue").val('0').hide(); $(".modaltitle").val('').hide(); $(".modal-delete").hide(); $("textarea#notes").val(login.user.notes).hide();
		}, 200);
	}
	...
	$("#savingsvalue").bind("keyup keydown keypress select click", function() {
		login.savings.freeMoney = $("#savingsvalue").autoNumeric("get"); moveRuble();
	}).keyup(function(e) {
		if (e.which == 13) {
			this.blur()
		}
	...
	$("#rublesign").on("click", function() {
		switch (login.user.checkedCurr) {
		case "RUB":
			login.user.checkedCurr = "EUR"; $("#rublesign").css({
				"background-position" : "-386px 0"
			});
			break;
		case "EUR":
			login.user.checkedCurr = "USD"; $("#rublesign").css({
				"background-position" : "-354px 0"
			});
			break;
		case "USD":
			login.user.checkedCurr = "RUB"; $("#rublesign").css({
				"background-position" : "-150px 0"
			});
			break;
		}
		changeCurrency();
		$("#savingsvalue").autoNumeric('set', Math.round(login.savings.freeMoney));
		moveRuble();
	...
	$(".notes-save").click(function() {
		login.user.notes = $("textarea#notes").val();
		turnOffModal();
		jsonDataSave();
	});
	...
	function launchStatistic() {
		if ($("#incomeslider").children().length > 0 && $("#expenseslider").children().length > 0) {
			...
			login.savings.percent = $("#percentvalue").autoNumeric("get");
			login.savings.deposit = $("#deposit").prop("checked");
			login.savings.capitalization = $("#capitalization").prop("checked");
		...
		if (global.savePermit) {
			$.ajax({
				...
				data : JSON.stringify({
					...
					saving : {
						amount : Math.ceil(login.savings.freeMoney),
						capitalization : login.savings.capitalization,
						deposit : login.savings.deposit,
						currency : login.user.checkedCurr,
						interest : login.savings.percent
					}
				}),
				success : function() {
	...
	function fadeStatistic() {
		...
		$("#savingsvalue").autoNumeric('set', login.savings.freeMoney);
		moveRuble();
		...
		setTimeout(function() {
			drawChartLine(91); $(".bottompage").css({
				"display" : "none"
			});
		}, 500);
	}
